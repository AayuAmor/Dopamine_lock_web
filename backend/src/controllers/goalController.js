...
const { allowedCategories, allowedPriorities, allowedStatuses } = require('../middleware/goalValidation')
...
function parseFilters(query) {
  const filters = {}
  if (query.status) {
    if (!allowedStatuses.has(query.status)) return { error: 'Goal status filter is invalid' }
    filters.status = query.status
  }
  if (query.category) {
    if (!allowedCategories.has(query.category)) return { error: 'Goal category filter is invalid' }
    filters.category = query.category
  }
  if (query.priority) {
    if (!allowedPriorities.has(query.priority)) return { error: 'Goal priority filter is invalid' }
    filters.priority = query.priority
  }
  if (query.archived !== undefined) {
    const archived = parseArchived(query.archived)
    if (archived === undefined) return { error: 'Archived filter must be true or false' }
...
async function updateGoalProgress(req, res) {
  try {
    const goalId = parseGoalId(req, res)
    if (!goalId) return undefined
    const goal = await updateGoalProgressById(req.user.id, goalId, req.goalProgressData)
    if (!goal) return res.status(404).json({ message: 'Goal not found' })
    return res.json({ goal })
  } catch (error) {
...
async function connectMission(req, res) {
  try {
    ...
    if (result.notFound === 'goal') return res.status(404).json({ message: 'Goal not found' })
    if (result.notFound === 'mission') return res.status(404).json({ message: 'Mission not found' })
